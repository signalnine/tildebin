...
- `baremetal_binfmt_misc_audit.py`: Audit binfmt_misc registered binary format handlers for security, detecting QEMU user-mode emulation, Wine handlers, and other registered interpreters that expand attack surface on production servers - useful for security compliance and container host hardening
...
- `baremetal_pre_maintenance_checker.py`: Pre-maintenance validation checker for baremetal systems - comprehensive pre-flight check before planned maintenance (reboots, upgrades, hardware changes) validating D-state processes, filesystem integrity, memory pressure, disk health, network connectivity, NFS mounts, systemd units, kernel taint, and recent OOM kills
...
- `baremetal_tcp_connection_monitor.py`: Monitor TCP connection states with per-process mapping to detect connection leaks (CLOSE_WAIT accumulation), port exhaustion risk (TIME_WAIT buildup), and identify processes with high connection counts by parsing /proc/net/tcp and mapping inodes to PIDs
...
- `baremetal_file_integrity_monitor.py`: Monitor critical system files for unauthorized modifications using cryptographic checksums (SHA256/SHA512) - supports baseline creation, verification, and reporting for security compliance (PCI-DSS, HIPAA, SOC2), rootkit/malware detection, and configuration drift monitoring
...
- `baremetal_container_image_cleanup_analyzer.py`: Analyze container image storage and cleanup needs across Docker, containerd, and podman - identifies dangling images, unused images, build cache usage, and estimates reclaimable space to help plan garbage collection before disk pressure
...
- `baremetal_process_tree_depth_monitor.py`: Monitor process tree depth to detect fork bombs and runaway process spawning by analyzing process hierarchy depth and child counts with configurable thresholds
...
- `k8s_version_skew_checker.py`: Check Kubernetes version skew policy compliance between kube-apiserver and kubelets, detecting nodes that exceed the N-3 minor version limit and control plane components exceeding N-1, essential for cluster upgrade planning and supportability

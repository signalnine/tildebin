...
This script verifies that kdump is properly configured to capture kernel crash
dumps for post-mortem analysis. Critical for large-scale baremetal environments
where kernel panics need to be debugged without physical access.
...
- kdump configuration file validation
- Crash dump directory permissions
- Recent crash dump files
- Fadump status (PowerPC systems)
...
def check_crashkernel_reservation():
    ...
    Returns:
        dict: Crashkernel reservation information
    """
    info = {
        'reserved': False,
        'parameter': None,
...
def check_dump_target(config):
    """Determine dump target from configuration.

    Args:
        config: kdump configuration dict

    Returns:
        dict: Dump target information
    """
    target = {
        'type': 'local',
        'path': '/var/crash',
...
def check_dump_directory(path):
    ...
    Returns:
        dict: Directory status information
    """
    info = {
        'path': path,
        'exists': False,
...
def find_recent_crashes(dump_path, max_files=10):
    ...
    Returns:
        list: List of crash dump file info
    """
    crashes = []
    ...
    try:
        ...
        patterns = [
            ...
            os.path.join(dump_path, '*/'),  # Timestamped directories
        ]
...
def check_fadump():
    ...
    Returns:
        dict: Fadump status information
    """
    info = {
        'supported': False,
        'enabled': False,
...
def check_kexec_loaded():
    """Check if kexec crash kernel is loaded.

    Returns:
        dict: Kexec status information
    """
    info = {
        'loaded': False,
...
def analyze_configuration(service, crashkernel, config, target, directory,
                          memory, kexec, fadump):
...
def output_plain(data, args):
    ...
    if not args.warn_only:
        ...
        ck = data['crashkernel']
        if ck['reserved']:
            print(f"Crashkernel: {ck['parameter']}")
        else:
        ...
        if data['recent_crashes']:
            print(f"\nRecent Crash Dumps ({len(data['recent_crashes'])} found):")
            for crash in data['recent_crashes'][:5]:
...
def output_table(data, args):
    ...
    if not args.warn_only:
        print("=" * 70)
        print("KDUMP CONFIGURATION STATUS")
        print("=" * 70)
        print()
        ...
        service = data['service']
        print(f"{'Service Installed':<30} {'Yes' if service['installed'] else 'No'}")
        print(f"{'Service Enabled':<30} {'Yes' if service['enabled'] else 'No'}")
        print(f"{'Service Active':<30} {'Yes' if service['active'] else 'No'}")

        ck = data['crashkernel']
        print(f"{'Crashkernel Reserved':<30} {'Yes' if ck['reserved'] else 'No'}")
        if ck['reserved']:
            print(f"{'Crashkernel Size':<30} {ck['size'] or ck['parameter']}")

        kexec = data['kexec']
        print(f"{'Crash Kernel Loaded':<30} {'Yes' if kexec['loaded'] else 'No'}")

        target = data['dump_target']
        print(f"{'Dump Target Type':<30} {target['type']}")
        print(f"{'Dump Path':<30} {target['path']}")

        directory = data['dump_directory']
        if directory['exists']:
            print(f"{'Available Space':<30} {format_bytes(directory['available_bytes'])}")
            print(f"{'Disk Usage':<30} {directory['used_percent']:.1f}%")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
  %(prog)s                      # Check kdump configuration
...
Kdump setup requirements:
  1. Install kdump package (kexec-tools on RHEL/CentOS, kdump-tools on Debian)
  2. Add crashkernel=256M (or auto) to kernel command line
  3. Configure /etc/kdump.conf with dump target
  4. Enable and start kdump service

Testing kdump (DANGEROUS - causes system crash):
...
Exit codes:
  ...
  2 - Usage error or system files not accessible
    ...
    parser.add_argument(
        '--dump-path',
        default='/var/crash',
        help='Path to check for crash dumps (default: %(default)s)'
    )
    ...
    if not os.path.exists('/proc'):
        print("Error: /proc not found - requires Linux", file=sys.stderr)
        sys.exit(2)

    # Gather information
    service = check_kdump_service()
    ...
    issues = analyze_configuration(
        service, crashkernel, config, target, directory, memory, kexec, fadump
    )

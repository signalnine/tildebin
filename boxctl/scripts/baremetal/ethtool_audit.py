...
def run(args: list[str], output: Output, context: Context) -> int:
    ...
    if opts.format == "json":
        ...
        if not opts.warn_only or has_issues:
            lines = []
            lines.append("Network Interface Ethtool Audit")
            lines.append("=" * 70)
            lines.append("")
            ...
            for finding in all_findings:
                ...
                lines.append(f"[{symbol}] {iface} - {status}")
                lines.append(f"    Driver: {driver} {version}")
                lines.append(f"    Speed: {speed}, MTU: {info.get('mtu', 'N/A')}")

                # Ring buffers if verbose or issues
                if opts.verbose and "ring_buffers" in info:
                    ...
                    lines.append(
                        f"    Ring buffers: RX={rx_cur}/{rx_max}, TX={tx_cur}/{tx_max}"
                    )

                # Offloads if verbose
                if opts.verbose and "offloads" in info:
                    offloads = info["offloads"]
                    enabled = [k for k, v in offloads.items() if v]
                    disabled = [k for k, v in offloads.items() if not v]
                    if enabled:
                        lines.append(f"    Offloads ON: {', '.join(enabled[:5])}")
                    if disabled:
                        lines.append(f"    Offloads OFF: {', '.join(disabled[:5])}")
            ...
            lines.append("-" * 70)
            lines.append(
                f"Summary: {summary['total']} interfaces - "
                f"{summary['healthy']} healthy, "
                f"{summary['with_warnings']} warnings, "
                f"{summary['with_issues']} issues"
            )

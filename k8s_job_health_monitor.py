#!/usr/bin/env python3
"""
Monitor Kubernetes Job and CronJob health and execution patterns.
...
- Resource consumption of active jobs
- CronJobs with consecutive failures

Useful for managing batch workloads on baremetal clusters where
failed jobs can accumulate and consume cluster resources.
...
def get_jobs(namespace: Optional[str] = None) -> List[Dict[str, Any]]:
...
def get_cronjobs(namespace: Optional[str] = None) -> List[Dict[str, Any]]:
    ...
    if succeeded > 0:
        ...
        if start_time:
            ...
            if running_seconds > 86400:  # 24 hours
                has_issue = True
                issue_details.append(f"Running for {running_seconds/3600:.1f} hours")
    else:
        job_status = 'Pending'
...
def analyze_cronjob(cronjob: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if last_schedule:
        time_since_schedule = (datetime.now(timezone.utc) - last_schedule).total_seconds()
        ...
        if time_since_schedule > 90000 and not suspended:  # 25 hours
            issue_details.append(f"Not scheduled for {time_since_schedule/3600:.1f} hours")
            has_issue = True
...
def output_plain(jobs_data: List[Dict], cronjobs_data: List[Dict], warn_only: bool):
    """Plain text output"""
    # Jobs
    if jobs_data:
        print("=== Jobs ===")
        for job in jobs_data:
            if warn_only and not job['has_issue']:
                continue
            ...
            print(f"  Succeeded: {job['succeeded']} | Failed: {job['failed']} | Active: {job['active']}")
    ...
    if cronjobs_data:
        ...
        for cj in cronjobs_data:
            if warn_only and not cj['has_issue']:
                continue
...
def output_json(jobs_data: List[Dict], cronjobs_data: List[Dict], warn_only: bool):
    """JSON output"""
    if warn_only:
...
def output_table(jobs_data: List[Dict], cronjobs_data: List[Dict], warn_only: bool):
    """Tabular output"""
    # Jobs table
    if jobs_data:
        print("=== Jobs ===")
        print(f"{'Namespace':<20} {'Name':<30} {'Status':<12} {'Age':<8} {'S/F/A':<10} {'Issues'}")
        print("-" * 120)
        ...
        for job in jobs_data:
            ...
            print(f"{job['namespace']:<20} {job['name']:<30} {job['status']:<12} "
    ...
    if cronjobs_data:
        print("=== CronJobs ===")
        print(f"{'Namespace':<20} {'Name':<30} {'Schedule':<15} {'Active':<8} {'Issues'}")
        print("-" * 110)
        ...
        for cj in cronjobs_data:
            ...
            schedule = cj['schedule'][:14] if len(cj['schedule']) > 14 else cj['schedule']
            issues_str = ', '.join(cj['issues']) if cj['issues'] else '-'

            print(f"{cj['namespace']:<20} {cj['name']:<30} {schedule:<15} "
...
Examples:
  ...
  %(prog)s --format table --warn-only
    ...
    if not args.skip_cronjobs:
        cronjobs = get_cronjobs(args.namespace)
        cronjobs_data = [analyze_cronjob(cj) for cj in cronjobs]

    # Output results
    if args.format == 'json':
        output_json(jobs_data, cronjobs_data, args.warn_only)
    elif args.format == 'table':
        output_table(jobs_data, cronjobs_data, args.warn_only)
    else:
        output_plain(jobs_data, cronjobs_data, args.warn_only)

    # Determine exit code
    has_issues = any(j['has_issue'] for j in jobs_data) or \
                 any(cj['has_issue'] for cj in cronjobs_data)

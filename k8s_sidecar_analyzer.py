...
- Missing sidecar containers where expected (injection failures)

Useful for:
- Understanding service mesh overhead across the cluster
- Identifying sidecar injection failures
- Auditing sidecar resource consumption
- Detecting pods stuck due to sidecar issues
- Capacity planning for sidecar workloads
...
def get_pods(namespace: Optional[str] = None) -> Dict[str, Any]:
    """Get all pods with full details."""
    cmd = ['get', 'pods', '-o', 'json']
    if namespace:
...
def analyze_pod(pod: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Also check namespace annotations (would need separate call)
    ...
    container_statuses = {
        s['name']: s for s in status.get('containerStatuses', [])
    }
    ...
    for container in containers:
        ...
        container_info = {
            'name': name,
            'image': image.split('/')[-1][:50],  # Truncate for display
...
def find_issues(pod_info: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    for sidecar in pod_info['sidecars']:
        ...
        if sidecar['restart_count'] >= 3:
            issues.append({
                ...
                'message': f"Sidecar '{name}' has restarted {sidecar['restart_count']} times",
            })
    ...
    # Check for resource imbalance (sidecar using more than main containers)
    ...
    if sidecar_cpu > main_cpu and main_cpu > 0:
        issues.append({
            ...
            'severity': 'INFO',
            'message': f"Sidecar CPU requests ({format_cpu(sidecar_cpu)}) exceed main container ({format_cpu(main_cpu)})",
        })
    ...
    if sidecar_mem > main_mem and main_mem > 0:
        issues.append({
            ...
            'severity': 'INFO',
            'message': f"Sidecar memory requests ({format_memory(sidecar_mem)}) exceed main container ({format_memory(main_mem)})",
        })
    
    return issues


def format_output_plain(pods_with_sidecars: List[Dict[str, Any]], 
    ...
    if summary['sidecar_types']:
        print("\nSidecar Types Found:")
        for stype, count in sorted(summary['sidecar_types'].items(), 
                                    key=lambda x: x[1], reverse=True):
            print(f"  {stype}: {count}")
    
    if summary['total_resources']['cpu_request'] > 0:
        print("\nTotal Sidecar Resource Usage:")
        print(f"  CPU Requests: {format_cpu(summary['total_resources']['cpu_request'])}")
        print(f"  CPU Limits:   {format_cpu(summary['total_resources']['cpu_limit'])}")
        print(f"  Mem Requests: {format_memory(summary['total_resources']['memory_request'])}")
        print(f"  Mem Limits:   {format_memory(summary['total_resources']['memory_limit'])}")
    ...
    elif not warn_only and pods_with_sidecars:
        print("\n" + "-" * 70)
        print("PODS WITH SIDECARS (no issues):")
        print("-" * 70)
        
        for pod in pods_with_sidecars[:20]:  # Limit to 20
            sidecar_types = [s['sidecar_type'] for s in pod['sidecars']]
            print(f"  {pod['namespace']}/{pod['name']}: {', '.join(sidecar_types)}")
...
def format_output_json(pods_with_sidecars: List[Dict[str, Any]], 
...
def format_output_table(pods_with_sidecars: List[Dict[str, Any]], 
                        summary: Dict[str, Any],
                        warn_only: bool = False) -> None:
    """Format output as table."""
    print(f"{'NAMESPACE':<20} {'POD':<30} {'SIDECARS':<20} {'ISSUE':<30}")
    print("=" * 105)
    ...
    for pod in pods_with_issues:
        sidecar_str = ', '.join([s['sidecar_type'][:8] for s in pod['sidecars']])
        for i, issue in enumerate(pod['issues']):
            if i == 0:
                print(f"{pod['namespace']:<20} "
                      f"{pod['name'][:28]:<30} "
                      f"{sidecar_str:<20} "
    ...
    print()
    print(f"Summary: {summary['pods_with_sidecars']} pods with sidecars, "
...
Examples:
  ...
  %(prog)s -n production              # Specific namespace
...
Exit codes:
  ...
  2 - Usage error or kubectl not available
    ...
    # Get all pods
    pods_data = get_pods(namespace=args.namespace)
    ...
    if args.warn_only and total_issues == 0:
        if args.format == 'json':
    ...
    # Output results
    if args.format == 'json':
        format_output_json(pods_with_sidecars, summary)
    elif args.format == 'table':
        format_output_table(pods_with_sidecars, summary, warn_only=args.warn_only)
    else:
        format_output_plain(pods_with_sidecars, summary, 

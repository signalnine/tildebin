#!/usr/bin/env python3
"""
Kubernetes API Server Latency Analyzer
...
This script performs a series of kubectl operations and measures their
response times to identify slow API operations. Useful for:
...
def measure_api_call(args, timeout=30):
    """
    ...
    try:
        ...
        return False, timeout * 1000, "Request timed out"
    except Exception as e:
        return False, 0, str(e)


def run_latency_tests(namespace=None, samples=3):
    """
    ...
    for test in tests:
        ...
        for _ in range(samples):
            success, latency_ms, error = measure_api_call(test['args'])
            if success:
...
def analyze_latency(results, warn_threshold_ms=500, critical_threshold_ms=2000):
    """
...
def format_plain(results, issues, warnings, warn_only=False):
    ...
    if not warn_only:
        ...
        lines.append("=" * 60)
        lines.append("")
        lines.append(f"{'Operation':<25} {'Avg (ms)':<12} {'Min (ms)':<12} {'Max (ms)':<12}")
        lines.append("-" * 60)
...
def format_json(results, issues, warnings):
    """Format output as JSON."""
    # Calculate overall statistics
    successful_results = [r for r in results if r['successful_samples'] > 0]

    if successful_results:
        overall_avg = sum(r['avg_latency_ms'] for r in successful_results) / len(successful_results)
...
def format_table(results, issues, warnings, warn_only=False):
    ...
    if not warn_only:
        ...
        lines.append("+" + "-" * 78 + "+")
        lines.append(f"| {'Operation':<24} | {'Avg (ms)':<10} | {'Min (ms)':<10} | {'Max (ms)':<10} | {'Status':<10} |")
        lines.append("+" + "-" * 78 + "+")
...
Examples:
  ...
  # Check specific namespace with lower thresholds
  %(prog)s -n production --warn-threshold 200 --critical-threshold 1000
...
Exit codes:
  0 - All API operations within acceptable latency
  ...
  2 - Usage error or kubectl not available
    ...
    # Run latency tests
    if args.verbose:
        print(f"Running {args.samples} samples per operation...", file=sys.stderr)

    results = run_latency_tests(args.namespace, args.samples)

    # Analyze results
    issues, warnings = analyze_latency(
        results,
        warn_threshold_ms=args.warn_threshold,
        critical_threshold_ms=args.critical_threshold
    )

    # Format output
    if args.format == 'json':
        output = format_json(results, issues, warnings)
    elif args.format == 'table':
        output = format_table(results, issues, warnings, args.warn_only)
    else:
        output = format_plain(results, issues, warnings, args.warn_only)

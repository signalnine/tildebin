#!/usr/bin/env python3
"""
Test script for baremetal_cpu_time_analyzer.py functionality.
Tests argument parsing and error handling without requiring specific CPU states.
"""
...
def test_help_message():
    ...
    return_code, stdout, stderr = run_command(
        [sys.executable, 'baremetal_cpu_time_analyzer.py', '--help']
    )

    if return_code == 0 and 'cpu' in stdout.lower() and 'steal' in stdout.lower():
...
def test_format_option_plain():
    ...
    return_code, stdout, stderr = run_command(
        [sys.executable, 'baremetal_cpu_time_analyzer.py', '--format', 'plain']
    )
...
def test_format_option_json():
    ...
    return_code, stdout, stderr = run_command(
        [sys.executable, 'baremetal_cpu_time_analyzer.py', '--format', 'json']
    )
    ...
    if return_code in [0, 1]:
        try:
            ...
            if 'summary' in data and 'aggregate' in data and 'per_cpu' in data:
...
def test_format_option_table():
    ...
    return_code, stdout, stderr = run_command(
        [sys.executable, 'baremetal_cpu_time_analyzer.py', '--format', 'table']
    )
...
def test_invalid_format():
    ...
    return_code, stdout, stderr = run_command(
        [sys.executable, 'baremetal_cpu_time_analyzer.py', '--format', 'invalid']
    )
...
def test_verbose_flag():
    ...
    return_code, stdout, stderr = run_command(
        [sys.executable, 'baremetal_cpu_time_analyzer.py', '-v']
    )
...
def test_warn_only_flag():
    ...
    return_code, stdout, stderr = run_command(
        [sys.executable, 'baremetal_cpu_time_analyzer.py', '--warn-only']
    )
...
def test_threshold_options():
    ...
    return_code, stdout, stderr = run_command(
        [sys.executable, 'baremetal_cpu_time_analyzer.py',
         '--steal-warn', '3',
         '--steal-crit', '10',
         '--iowait-warn', '5',
         '--iowait-crit', '20']
...
def test_json_structure():
    ...
    try:
        ...
        required_summary_keys = ['cpu_count', 'issue_count', 'critical_count', 'warning_count']
        ...
        if 'aggregate' not in data:
            print("[FAIL] JSON structure test failed: missing 'aggregate'")
            return False

        agg = data['aggregate']
        required_agg_keys = ['user', 'system', 'idle', 'iowait', 'steal']
        for key in required_agg_keys:
            if key not in agg:
                print(f"[FAIL] JSON structure test failed: missing aggregate key '{key}'")
                return False
...
def test_plain_output_contains_metrics():
    """Test that plain output contains expected CPU metrics"""
    return_code, stdout, stderr = run_command(
        [sys.executable, 'baremetal_cpu_time_analyzer.py', '--format', 'plain']
    )
    ...
    expected_labels = ['User:', 'System:', 'Idle:', 'I/O Wait:', 'Steal:']
...
def test_imbalance_threshold():
    """Test that imbalance threshold options are accepted"""
    return_code, stdout, stderr = run_command(
        [sys.executable, 'baremetal_cpu_time_analyzer.py',
         '--imbalance-warn', '30',
         '--imbalance-crit', '50']
    ...
    if return_code in [0, 1, 2]:  # Valid exit codes
        print("[PASS] Imbalance threshold options test passed")
        return True
    else:
        print(f"[FAIL] Imbalance threshold options test failed: unexpected return code {return_code}")
        return False


if __name__ == "__main__":
    print("Testing baremetal_cpu_time_analyzer.py...")
    print("=" * 60)

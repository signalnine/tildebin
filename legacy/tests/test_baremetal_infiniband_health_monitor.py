#!/usr/bin/env python3
"""
Test script for baremetal_infiniband_health_monitor.py functionality.
Tests argument parsing and output formats without requiring InfiniBand hardware.
"""
...
def test_help_message():
    ...
    return_code, stdout, stderr = run_command(
        [sys.executable, 'baremetal_infiniband_health_monitor.py', '--help']
    )

    if return_code == 0 and 'infiniband' in stdout.lower():
...
def test_help_contains_rdma():
    """Test that help mentions RDMA."""
    return_code, stdout, stderr = run_command(
        [sys.executable, 'baremetal_infiniband_health_monitor.py', '--help']
    )

    if return_code == 0 and 'rdma' in stdout.lower():
        print("[PASS] Help contains RDMA test passed")
        return True
    else:
        print(f"[FAIL] Help should mention RDMA")
        return False
...
def test_invalid_arguments():
    ...
    return_code, stdout, stderr = run_command(
        [sys.executable, 'baremetal_infiniband_health_monitor.py', '--invalid-flag']
    )
...
def test_format_option_recognized():
    ...
    return_code, stdout, stderr = run_command(
        [sys.executable, 'baremetal_infiniband_health_monitor.py', '--format', 'invalid']
    )
...
def test_plain_output_format():
    ...
    return_code, stdout, stderr = run_command(
        [sys.executable, 'baremetal_infiniband_health_monitor.py']
    )

    # Exit code 0 or 1 = success (healthy or issues), 2 = no IB hardware
    if return_code in [0, 1]:
        if 'infiniband' in stdout.lower() or 'device' in stdout.lower():
        ...
        # No IB hardware - expected on most systems
        if 'not found' in stderr.lower() or 'no infiniband' in stderr.lower():
            print("[SKIP] Plain output format test skipped (no IB hardware)")
            return True
...
def test_json_output_format():
    ...
    return_code, stdout, stderr = run_command(
        [sys.executable, 'baremetal_infiniband_health_monitor.py', '--format', 'json']
    )

    if return_code == 2:
        # No IB hardware - check for valid JSON error output
        if stdout.strip():
            try:
                data = json.loads(stdout)
                if 'healthy' in data and data['healthy'] is False:
                    print("[PASS] JSON output format test passed (no IB hardware, valid JSON)")
                    return True
            except json.JSONDecodeError:
                pass
        print("[SKIP] JSON output format test skipped (no IB hardware)")
        return True
    ...
    try:
        ...
        required_keys = ['devices', 'rdma_devices', 'issues', 'healthy']
...
def test_table_output_format():
    ...
    return_code, stdout, stderr = run_command(
        [sys.executable, 'baremetal_infiniband_health_monitor.py', '--format', 'table']
    )

    if return_code == 2:
        print("[SKIP] Table output format test skipped (no IB hardware)")
        return True

    if return_code in [0, 1] and ('Device' in stdout or 'Port' in stdout or 'no devices' in stdout.lower()):
...
def test_verbose_mode():
    ...
    return_code, stdout, stderr = run_command(
        [sys.executable, 'baremetal_infiniband_health_monitor.py', '--verbose']
    )

    if return_code == 2:
        print("[SKIP] Verbose mode test skipped (no IB hardware)")
        return True
...
def test_warn_only_mode():
    """Test warn-only mode."""
    return_code, stdout, stderr = run_command(
        [sys.executable, 'baremetal_infiniband_health_monitor.py', '--warn-only']
    )
...
def test_json_has_healthy_flag():
    ...
    if return_code == 2 and not stdout.strip():
        print("[SKIP] JSON healthy flag test skipped (no IB hardware)")
        return True
    ...
    try:
        ...
        if return_code == 2:
            print("[SKIP] JSON healthy flag test skipped (no IB hardware, no JSON output)")
            return True
        print(f"[FAIL] JSON healthy flag test failed: {e}")
        return False


def test_json_has_devices():
    """Test JSON output includes devices list."""
    return_code, stdout, stderr = run_command(
        [sys.executable, 'baremetal_infiniband_health_monitor.py', '--format', 'json']
    )

    if return_code == 2 and not stdout.strip():
        print("[SKIP] JSON devices test skipped (no IB hardware)")
        return True
    ...
    try:
        ...
        if 'devices' not in data:
            print("[FAIL] JSON output missing 'devices'")
            return False

        if not isinstance(data['devices'], list):
            print("[FAIL] 'devices' is not a list")
            return False

        print("[PASS] JSON has devices test passed")
        return True
    except json.JSONDecodeError as e:
        if return_code == 2:
            print("[SKIP] JSON devices test skipped (no valid JSON)")
            return True
        print(f"[FAIL] JSON devices test failed: {e}")
        return False


def test_json_has_rdma_devices():
    """Test JSON output includes rdma_devices list."""
    return_code, stdout, stderr = run_command(
        [sys.executable, 'baremetal_infiniband_health_monitor.py', '--format', 'json']
    )

    if return_code == 2 and not stdout.strip():
        print("[SKIP] JSON rdma_devices test skipped (no IB hardware)")
        return True

    try:
        data = json.loads(stdout)

        if 'rdma_devices' not in data:
            print("[FAIL] JSON output missing 'rdma_devices'")
            return False

        if not isinstance(data['rdma_devices'], list):
            print("[FAIL] 'rdma_devices' is not a list")
            return False

        print("[PASS] JSON has rdma_devices test passed")
        return True
    except json.JSONDecodeError as e:
        if return_code == 2:
            print("[SKIP] JSON rdma_devices test skipped (no valid JSON)")
            return True
        print(f"[FAIL] JSON rdma_devices test failed: {e}")
        return False
...
def test_json_has_issues():
    ...
    if return_code == 2 and not stdout.strip():
        print("[SKIP] JSON issues test skipped (no IB hardware)")
        return True
...
def test_help_contains_troubleshooting():
    ...
    if return_code == 0 and ('troubleshooting' in stdout.lower() or 'perfquery' in stdout):
...
def test_warn_only_json():
    ...
    return_code, stdout, stderr = run_command(
        [sys.executable, 'baremetal_infiniband_health_monitor.py', '--warn-only', '--format', 'json']
    )
...
def test_short_verbose_flag():
    ...
    return_code, stdout, stderr = run_command(
        [sys.executable, 'baremetal_infiniband_health_monitor.py', '-v']
    )
...
def test_short_warn_only_flag():
    ...
    return_code, stdout, stderr = run_command(
        [sys.executable, 'baremetal_infiniband_health_monitor.py', '-w']
    )
...
def test_combined_flags():
    ...
    return_code, stdout, stderr = run_command(
        [sys.executable, 'baremetal_infiniband_health_monitor.py', '-v', '--format', 'json']
    )
...
def test_json_has_summary():
    ...
    if return_code == 2 and not stdout.strip():
        print("[SKIP] JSON summary test skipped (no IB hardware)")
        return True
    ...
    try:
        ...
        required = ['device_count', 'error_count', 'warning_count']
...
def test_error_message_helpful():
    """Test that error messages are helpful when no IB hardware."""
    return_code, stdout, stderr = run_command(
        [sys.executable, 'baremetal_infiniband_health_monitor.py']
    )
    ...
    # If we have IB hardware, test passes
    print("[PASS] Error message helpful test passed (IB hardware present)")
    return True


if __name__ == "__main__":
    print("Testing baremetal_infiniband_health_monitor.py...")
    print()

    tests = [
        test_help_message,
        test_help_contains_rdma,
        ...
        test_json_has_devices,
        test_json_has_rdma_devices,

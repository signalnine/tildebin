#!/usr/bin/env python3
"""
Tests for hardware_temperature_monitor.py

These tests verify the script's argument parsing and basic functionality
without requiring actual hardware sensors.
"""
...
def test_help_message():
    ...
    if 'Monitor hardware temperature sensors' not in stdout:
...
def test_format_options():
    ...
    for fmt in ['plain', 'json', 'table']:
        ...
        # Script should run (may exit with 2 if sensors not available, which is OK)
...
def test_no_sensors_handling():
    """Test graceful handling when sensors command is not available."""
    return_code, stdout, stderr = run_command([
        sys.executable,
        'hardware_temperature_monitor.py'
    ])

    # Should either work (0, 1) or report missing sensors (2)
    if return_code not in [0, 1, 2]:
        print(f"[FAIL] No sensors handling test: Unexpected return code {return_code}")
        return False

    # If sensors not found, should have helpful error message
    if return_code == 2 and 'sensors' in stderr.lower():
        if 'lm-sensors' not in stderr.lower() and 'lm_sensors' not in stderr.lower():
            print("[FAIL] No sensors handling test: Missing helpful installation message")
            return False

    print("[PASS] No sensors handling test")
    return True
...
def main():
    ...
    print("Running hardware_temperature_monitor.py tests...")
    print()

#!/usr/bin/env python3
"""
Tests for k8s_pod_image_registry_audit.py
...
def test_help_message():
    ...
    if 'Audit Kubernetes pod container image registries' not in stdout:
    ...
    if '--approved-registry' not in stdout:
        print("[FAIL] Help message test: --approved-registry option not found")
        return False

    if '--block-public-in-prod' not in stdout:
        print("[FAIL] Help message test: --block-public-in-prod option not found")
        return False
...
def test_approved_registry_option():
    """Test that --approved-registry option is recognized."""
    return_code, stdout, stderr = run_command([
        sys.executable,
        'k8s_pod_image_registry_audit.py',
        '--approved-registry', 'my-registry.example.com'
    ])

    if return_code not in [0, 1, 2]:
        print(f"[FAIL] Approved registry option test: Unexpected return code {return_code}")
        return False

    if 'unrecognized arguments' in stderr.lower():
        print("[FAIL] Approved registry option test: --approved-registry option not recognized")
        return False

    print("[PASS] Approved registry option test")
    return True


def test_skip_approval_check_flag():
    """Test that --skip-approval-check flag is recognized."""
    return_code, stdout, stderr = run_command([
        sys.executable,
        'k8s_pod_image_registry_audit.py',
        '--skip-approval-check'
    ...
    if return_code not in [0, 1, 2]:
        print(f"[FAIL] Skip approval check flag test: Unexpected return code {return_code}")
        return False

    if 'unrecognized arguments' in stderr.lower():
        print("[FAIL] Skip approval check flag test: Flag not recognized")
        return False

    print("[PASS] Skip approval check flag test")
    return True


def test_block_public_in_prod_flag():
    """Test that --block-public-in-prod flag is recognized."""
    return_code, stdout, stderr = run_command([
        sys.executable,
        'k8s_pod_image_registry_audit.py',
    ...
    if return_code not in [0, 1, 2]:
        print(f"[FAIL] Block public in prod flag test: Unexpected return code {return_code}")
        return False

    if 'unrecognized arguments' in stderr.lower():
        print("[FAIL] Block public in prod flag test: Flag not recognized")
        return False

    print("[PASS] Block public in prod flag test")
    return True


def test_production_namespace_option():
    """Test that --production-namespace option is recognized."""
    return_code, stdout, stderr = run_command([
        sys.executable,
        'k8s_pod_image_registry_audit.py',
        '--production-namespace', 'staging'
    ])

    if return_code not in [0, 1, 2]:
        print(f"[FAIL] Production namespace option test: Unexpected return code {return_code}")
        return False

    if 'unrecognized arguments' in stderr.lower():
        print("[FAIL] Production namespace option test: --production-namespace option not recognized")
        return False

    print("[PASS] Production namespace option test")
    return True


def test_no_stats_flag():
    """Test that --no-stats flag is recognized."""
    return_code, stdout, stderr = run_command([
        sys.executable,
        'k8s_pod_image_registry_audit.py',
        '--no-stats'
    ...
    if return_code not in [0, 1, 2]:
        print(f"[FAIL] No stats flag test: Unexpected return code {return_code}")
        return False

    if 'unrecognized arguments' in stderr.lower():
        print("[FAIL] No stats flag test: Flag not recognized")
        return False

    print("[PASS] No stats flag test")
    return True
...
def test_multiple_approved_registries():
    """Test that multiple --approved-registry options can be specified."""
    return_code, stdout, stderr = run_command([
        sys.executable,
        'k8s_pod_image_registry_audit.py',
        '--approved-registry', 'registry1.example.com',
        '--approved-registry', 'registry2.example.com'
    ])

    if return_code not in [0, 1, 2]:
        print(f"[FAIL] Multiple approved registries test: Unexpected return code {return_code}")
        return False

    if 'unrecognized arguments' in stderr.lower():
        print("[FAIL] Multiple approved registries test: Multiple --approved-registry not recognized")
        return False

    print("[PASS] Multiple approved registries test")
    return True


def test_multiple_production_namespaces():
    """Test that multiple --production-namespace options can be specified."""
    return_code, stdout, stderr = run_command([
        sys.executable,
        'k8s_pod_image_registry_audit.py',
        '--production-namespace', 'staging',
        '--production-namespace', 'canary'
    ])

    if return_code not in [0, 1, 2]:
        print(f"[FAIL] Multiple production namespaces test: Unexpected return code {return_code}")
        return False

    if 'unrecognized arguments' in stderr.lower():
        print("[FAIL] Multiple production namespaces test: Multiple --production-namespace not recognized")
        return False

    print("[PASS] Multiple production namespaces test")
    return True
...
def main():
    ...
    print("Running k8s_pod_image_registry_audit.py tests...")
    print()
    ...
    tests = [
        ...
        test_approved_registry_option,
        test_skip_approval_check_flag,
        test_block_public_in_prod_flag,
        test_production_namespace_option,
        test_no_stats_flag,
        test_combined_options,
        test_invalid_format,
        test_no_kubectl_handling,
        test_short_option_aliases,
        test_multiple_approved_registries,
        test_multiple_production_namespaces,
    ]

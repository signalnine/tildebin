...
# boxctl:
#   category: k8s/batch
#   tags: [jobs, cronjobs, kubernetes, batch, status]
#   requires: [kubectl]
#   privilege: user
#   brief: Monitor Jobs and CronJobs status with detailed health info
#   related: [job_failures, job_health]

"""
Kubernetes Job status monitor - Track job completion and CronJob health.

Provides visibility into Job and CronJob health:
...
def parse_duration(start_time: str, completion_time: str | None = None) -> float | None:
...
def check_job_status(job: dict, max_duration_hours: int = 24) -> tuple:
    ...
    if active > 0 and start_time:
        ...
        if duration and duration > (max_duration_hours * 3600):
            issues.append(
                f"Job running for {format_duration(duration)} (exceeds {max_duration_hours}h threshold)"
            )
            is_healthy = False
    ...
    if failed > 0:
        if failed >= backoff_limit:
            issues.append(
                f"Job has {failed} failures (reached backoff limit of {backoff_limit})"
            )
            is_healthy = False
        else:
...
def check_cronjob_status(cronjob: dict) -> tuple:
    ...
    if len(active) > 1 and concurrency_policy == "Forbid":
        issues.append(
            f"Multiple active jobs ({len(active)}) with Forbid concurrency policy"
        )
        is_healthy = False
    ...
    if last_successful_time:
        ...
        if time_since_success and time_since_success > (48 * 3600):
            issues.append(
                f"No successful run in {format_duration(time_since_success)}"
            )
            is_healthy = False
...
def run(args: list[str], output: Output, context: Context) -> int:
    ...
    # Get jobs
    job_cmd = ["kubectl", "get", "jobs", "-o", "json"]
    if opts.namespace:
        job_cmd.extend(["-n", opts.namespace])
    else:
        job_cmd.append("--all-namespaces")

    try:
        result = context.run(job_cmd)
    ...
    if opts.format == "json":
        ...
        for job in jobs.get("items", []):
            ...
            is_healthy, issues, info = check_job_status(job, opts.max_duration)
            ...
            if opts.failed_only and is_healthy:
                continue
            if opts.warn_only and not issues:
                continue

            output_data["jobs"].append(job_info)
            if not is_healthy:
                has_issues = True
        ...
        for cj in cronjobs.get("items", []):
            ...
            output_data["cronjobs"].append(cj_info)
            if not is_healthy:
                has_issues = True
        ...
        if not job_items:
            ...
            for job in job_items:
                ...
                print(
                    f"    Status: {info['succeeded']}/{info['completions']} succeeded, "
        ...
        print(
            f"Summary: {healthy_jobs}/{total_jobs} jobs healthy, "
            f"{healthy_cronjobs}/{total_cronjobs} cronjobs healthy"
        )

    output.set_summary(
        f"jobs_healthy={healthy_jobs}, jobs_unhealthy={unhealthy_jobs}, "
        f"cronjobs_healthy={healthy_cronjobs}, cronjobs_unhealthy={unhealthy_cronjobs}"
    )

#!/usr/bin/env python3
"""
Test script for baremetal_bandwidth_monitor.py functionality.
...
def test_help_message():
    ...
    return_code, stdout, stderr = run_command(
        [sys.executable, 'baremetal_bandwidth_monitor.py', '--help']
    )

    if return_code == 0 and 'bandwidth' in stdout.lower():
...
def test_invalid_arguments():
    ...
    return_code, stdout, stderr = run_command(
        [sys.executable, 'baremetal_bandwidth_monitor.py', '--invalid-flag']
    )
...
def test_json_output_format():
    ...
    return_code, stdout, stderr = run_command(
        [sys.executable, 'baremetal_bandwidth_monitor.py', '--format', 'json', '-i', '0.1']
    )
...
def test_table_format():
    ...
    return_code, stdout, stderr = run_command(
        [sys.executable, 'baremetal_bandwidth_monitor.py', '--format', 'table', '-i', '0.1']
    )
...
def test_plain_format():
    ...
    return_code, stdout, stderr = run_command(
        [sys.executable, 'baremetal_bandwidth_monitor.py', '--format', 'plain', '-i', '0.1']
    )
...
def test_verbose_flag():
    ...
    return_code, stdout, stderr = run_command(
        [sys.executable, 'baremetal_bandwidth_monitor.py', '-v', '-i', '0.1']
    )
...
def test_warn_only_flag():
    ...
    return_code, stdout, stderr = run_command(
        [sys.executable, 'baremetal_bandwidth_monitor.py', '--warn-only', '-i', '0.1']
    )
...
def test_custom_interval():
    ...
    return_code, stdout, stderr = run_command(
        [sys.executable, 'baremetal_bandwidth_monitor.py', '-i', '0.1', '--format', 'json']
    )
    ...
    if return_code in [0, 1]:
        try:
            ...
            if data.get('sample_interval_sec') == 0.1:
                ...
                print(f"[FAIL] Interval not recorded correctly: {data.get('sample_interval_sec')}")
                return False
...
def test_invalid_interval():
    ...
    return_code, stdout, stderr = run_command(
        [sys.executable, 'baremetal_bandwidth_monitor.py', '-i', '0']
    )
...
def test_negative_interval():
    ...
    return_code, stdout, stderr = run_command(
        [sys.executable, 'baremetal_bandwidth_monitor.py', '-i', '-1']
    )
...
def test_custom_thresholds():
    ...
    return_code, stdout, stderr = run_command(
        [sys.executable, 'baremetal_bandwidth_monitor.py',
         '--warn', '50', '--crit', '80', '-i', '0.1']
    )
...
def test_exclude_down_flag():
    """Test exclude-down flag"""
    return_code, stdout, stderr = run_command(
        [sys.executable, 'baremetal_bandwidth_monitor.py',
         '--exclude-down', '-i', '0.1', '--format', 'json']
    )

    # Should succeed (0), detect issues (1), or missing deps (2)
    if return_code in [0, 1, 2]:
        print("[PASS] Exclude-down flag test passed")
        return True
    else:
        print(f"[FAIL] Exclude-down flag test failed with code {return_code}")
        return False


def test_interface_filter():
    """Test interface filter with loopback (always exists)"""
    return_code, stdout, stderr = run_command(
        [sys.executable, 'baremetal_bandwidth_monitor.py',
         '--interface', 'lo', '-i', '0.1', '--format', 'json']
    )

    # Should succeed (0) or detect issues (1) for loopback
    # Could be 2 if /proc not available
    if return_code in [0, 1]:
        try:
            data = json.loads(stdout)
            # Verify only loopback is present (if any interfaces)
            ifaces = [i['interface'] for i in data.get('interfaces', [])]
            if len(ifaces) == 0 or ifaces == ['lo']:
                print("[PASS] Interface filter test passed")
                return True
            else:
                print(f"[FAIL] Found unexpected interfaces: {ifaces}")
                return False
        except json.JSONDecodeError:
            print("[FAIL] Interface filter produced invalid JSON")
            return False
    elif return_code == 2:
        print("[PASS] Interface filter test passed (dependency or interface missing)")
        return True
    else:
        print(f"[FAIL] Interface filter test failed with code {return_code}")
        return False


def test_nonexistent_interface():
    """Test that nonexistent interface returns error"""
    return_code, stdout, stderr = run_command(
        [sys.executable, 'baremetal_bandwidth_monitor.py',
         '--interface', 'nonexistent_iface_xyz', '-i', '0.1']
    )
...
def test_exit_codes():
    ...
    return_code, stdout, stderr = run_command(
        [sys.executable, 'baremetal_bandwidth_monitor.py', '-i', '0.1']
    )
...
def test_json_summary_structure():
    ...
    if return_code in [0, 1]:
        try:
            ...
            expected_keys = ['total_interfaces', 'interfaces_up', 'warning_count', 'critical_count']
    ...
    if return_code in [0, 1]:
        try:
            ...
            if interfaces:
                iface = interfaces[0]
                expected_keys = [
                    'interface', 'operstate', 'rx_bytes_sec', 'tx_bytes_sec',
                    'rx_packets_sec', 'tx_packets_sec', 'rx_bits_sec', 'tx_bits_sec',
                    'status'
                ]
...
if __name__ == "__main__":
    print(f"Testing baremetal_bandwidth_monitor.py...")
    print("=" * 60)

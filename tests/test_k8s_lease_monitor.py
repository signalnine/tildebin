#!/usr/bin/env python3
"""Tests for k8s_lease_monitor.py"""
...
def test_help():
    ...
    result = subprocess.run(
        ["python3", "k8s_lease_monitor.py", "--help"],
        capture_output=True,
        text=True
    )
    assert result.returncode == 0, f"Help failed: {result.stderr}"
    assert "lease" in result.stdout.lower()
    assert "leader election" in result.stdout.lower()
...
def test_help_short():
    """Test -h flag"""
    result = subprocess.run(
        ["python3", "k8s_lease_monitor.py", "-h"],
        capture_output=True,
        text=True
    )
    assert result.returncode == 0, f"Help failed: {result.stderr}"
...
def test_format_argument():
    ...
    for fmt in ["plain", "json", "table"]:
        result = subprocess.run(
            ["python3", "k8s_lease_monitor.py", "--format", fmt],
            capture_output=True,
            text=True,
            timeout=15
        )
...
def test_invalid_format():
    ...
    result = subprocess.run(
        ["python3", "k8s_lease_monitor.py", "--format", "invalid"],
        capture_output=True,
        text=True,
        timeout=10
    )
...
def test_namespace_argument():
    ...
    result = subprocess.run(
        ["python3", "k8s_lease_monitor.py", "--namespace", "kube-system"],
        capture_output=True,
        text=True,
        timeout=15
    )
    ...
    # Try short form
    result = subprocess.run(
        ["python3", "k8s_lease_monitor.py", "-n", "default"],
        capture_output=True,
        text=True,
        timeout=15
    )
...
def test_verbose_flag():
    """Test --verbose flag"""
    result = subprocess.run(
        ["python3", "k8s_lease_monitor.py", "--verbose"],
        capture_output=True,
        text=True,
        timeout=15
    )
    ...
    # Try short form
    result = subprocess.run(
        ["python3", "k8s_lease_monitor.py", "-v"],
        capture_output=True,
        text=True,
        timeout=15
    )
...
def test_warn_only_flag():
    """Test --warn-only flag"""
    result = subprocess.run(
        ["python3", "k8s_lease_monitor.py", "--warn-only"],
        capture_output=True,
        text=True,
        timeout=15
    )
    ...
    # Try short form
    result = subprocess.run(
        ["python3", "k8s_lease_monitor.py", "-w"],
        capture_output=True,
        text=True,
        timeout=15
    )
...
def test_stale_threshold_argument():
    """Test --stale-threshold argument"""
    result = subprocess.run(
        ["python3", "k8s_lease_monitor.py", "--stale-threshold", "120"],
        capture_output=True,
        text=True,
        timeout=15
    )
    assert result.returncode in [0, 1, 2]
    print("PASS: --stale-threshold argument works")


def test_skip_node_leases_flag():
    """Test --skip-node-leases flag"""
    result = subprocess.run(
        ["python3", "k8s_lease_monitor.py", "--skip-node-leases"],
        capture_output=True,
        text=True,
        timeout=15
    )
    assert result.returncode in [0, 1, 2]
    print("PASS: --skip-node-leases flag works")


def test_check_orphans_flag():
    """Test --check-orphans flag"""
    result = subprocess.run(
        ["python3", "k8s_lease_monitor.py", "--check-orphans"],
        capture_output=True,
        text=True,
        timeout=15
    )
    assert result.returncode in [0, 1, 2]
    print("PASS: --check-orphans flag works")


def test_combined_arguments():
    """Test multiple arguments together"""
    result = subprocess.run(
        ["python3", "k8s_lease_monitor.py", "-n", "kube-system", "--format", "json", "-v"],
        capture_output=True,
        text=True,
        timeout=15
    )
...
def test_json_output_format():
    ...
    result = subprocess.run(
        ["python3", "k8s_lease_monitor.py", "--format", "json"],
        capture_output=True,
        text=True,
        timeout=15
    )
...
def test_json_summary_fields():
    ...
    if result.stdout.strip():
        ...
        assert "total_leases" in summary
        assert "leases_with_issues" in summary
...
def test_plain_output_format():
    ...
    result = subprocess.run(
        ["python3", "k8s_lease_monitor.py", "--format", "plain"],
        capture_output=True,
        text=True,
        timeout=15
    )
...
def test_table_output_format():
    ...
    result = subprocess.run(
        ["python3", "k8s_lease_monitor.py", "--format", "table"],
        capture_output=True,
        text=True,
        timeout=15
    )
...
def test_lease_types_documentation():
    """Test that help documents lease types"""
    result = subprocess.run(
        ["python3", "k8s_lease_monitor.py", "--help"],
        capture_output=True,
        text=True
    ...
    assert result.returncode == 0
    # Should mention lease types
    assert "control-plane" in result.stdout.lower() or "node-heartbeat" in result.stdout.lower()
    print("PASS: Help documents lease types")
...
def main():
    """Run all tests"""
    print("Running k8s_lease_monitor tests...")
    print()
    ...
    tests = [
        ...
        test_verbose_flag,
        test_warn_only_flag,
        test_stale_threshold_argument,
        test_skip_node_leases_flag,
        test_check_orphans_flag,
        ...
        test_lease_types_documentation,

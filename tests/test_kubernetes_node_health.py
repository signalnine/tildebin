#!/usr/bin/env python3
"""
Tests for kubernetes_node_health.py
...
def run_command(cmd_args, input_data=None):
    """Run the kubernetes_node_health.py script with given arguments."""
    cmd = [sys.executable, 'kubernetes_node_health.py'] + cmd_args
...
class TestKubernetesNodeHealth(unittest.TestCase):
    """Test cases for kubernetes_node_health.py"""

    def test_help_message(self):
        """Test that --help flag works and shows usage information."""
        returncode, stdout, stderr = run_command(['--help'])
        self.assertEqual(returncode, 0)
        self.assertIn('Kubernetes node health', stdout)
...
class TestScriptMetadata(unittest.TestCase):
    ...
    def test_script_has_shebang(self):
        ...
        with open('kubernetes_node_health.py', 'r') as f:
